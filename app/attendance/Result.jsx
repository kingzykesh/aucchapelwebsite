...
export default function AttendanceResult() {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-6">
        ...
        <a href="/login" className="text-blue-700 underline">Go back</a>
      </div>
    );
  }


  const grouped = { mbs: null, trh: null, sws: null };
  ...
  const percentage = Math.round((totalMarks / weeklyServices) * 100);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-14 px-6">
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-2xl p-10 pb-16">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-extrabold text-blue-800">{student.full_name}</h2>
        ...
        <h3 className="text-xl font-bold text-gray-800 mb-5">Service Attendance</h3>

        <div className="grid md:grid-cols-3 gap-6">
          <ServiceCard title="MBS (Monday Bible Study)" date={formatDate(mbs?.date)} mark={markValue(mbs?.mark)} />
          <ServiceCard title="TRH (Thursday Revival Hour)" date={formatDate(trh?.date)} mark={markValue(trh?.mark)} />
          <ServiceCard title="SWS (Sunday Worship Service)" date={formatDate(sws?.date)} mark={markValue(sws?.mark)} />
        </div>
        ...
        >
          <h3 className="text-2xl font-bold mb-2">Keep Growing in God’s Presence</h3>
